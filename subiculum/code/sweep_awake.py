...
PROJECT = "Sub awake response Training factorized readout 23-07-2024"
...
def load_data():
    ...
    train_loader, val_loader, test_loader = dataloader_from_mat(images_path_awake, v1_responses_path_awake, 50, 120, 64)
...
def configure_model(config, n_neurons):
    model = ConvModel(layers=config.layers, 
                      ...
                      fact_input1=fact_input_calc(config.input_kern, config.hidden_kern,config.layers,72),
                      fact_input2=fact_input_calc(config.input_kern, config.hidden_kern,config.layers,128))
    return model
...
def objective(config, seed=42):
    ...
    model = configure_model(config, n_neurons=147) 
    model.to(device)
