...
def neural_subset(responses: torch.Tensor, cell_type={"all"}, type_index=None, accepted_sessions={1,2,3}, session_index=None, response_quality={0,1,2})->torch.Tensor:
    ...
    if "HD" in cell_type:
        responses = responses[:, type_index==2]
        session_index = session_index[:, type_index==2] or None
        type_index = type_index[:, type_index!=2]
    if "FS" in cell_type:
        responses = responses[:, type_index==1]
        session_index = session_index[:, type_index==1] or None
        type_index = type_index[:, type_index!=1]
    if "NC" in cell_type:
        responses = responses[:, type_index==4]
        session_index = session_index[:, type_index==4] or None
        type_index = type_index[:, type_index!=4]
    if "slow" in cell_type:
        responses = responses[:, type_index==3]
        session_index = session_index[:, type_index==3] or None
        type_index = type_index[:, type_index!=3]
