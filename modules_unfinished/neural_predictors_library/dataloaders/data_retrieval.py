...
"""
These functions will be called first
"""
...
def retrieve_new_data(file_path: str, images_path: str,ids_path:str, device: torch.device):
    ...
    responses=torch.tensor(responses, dtype=torch.float32).to(device)
    ...
    images_list = [transform(img) for img in images]
    images = torch.stack(images_list).squeeze().to(device)
    ...
    id_session = torch.from_numpy(ids['rec'][:, 0]).to(device)
    
    from neural_predictors_library.constants import stim_list 
    test_boolean=torch.tensor(check_for_repeated_stims(stim_list))
    
    return responses, images, idx_cell_type, id_session, test_boolean
