...
def training_and_eval_with_lr(model, epochs, train_loader, test_loader, val_loader, device, save_model= False, lr=1e-1, gamma=1e-3, path_for_saving=None, early_stopping=True, Poisson=True, weight_decay=0, dropout=False, dropout_rate=0):
    ...
    if dropout==True:
        do = nn.Dropout(dropout_rate)
    else:
        do= None
    for epoch in trange(epochs):
        # Training loop
        loss = my_train_epoch(model, train_loader, optimizer, loss_fn, device, do)
        # Validation loop
        with torch.no_grad():
    ...
    # Evaluate the model
    model.eval()
    evaluate_model(model, test_loader, device,loss_fn)
